...
function BreadcrumbsBar({variant}) {
    ...
    const pathnames = decodeURI(location.pathname).split("/").filter(x=>x).slice(1);
        ...
        return(
            <Breadcrumbs separator={<ChevronRight />} aria-label="breadcrumb" >
            {
                pathnames.map((name, index) => {
                    ...
                    return isLast ? (
                        <SmallText variant="purpleAccent" weight="bold">{name}</SmallText>
    ...
    return (
          <Breadcrumbs separator={<ChevronRight />} aria-label="breadcrumb" >
            <BreadcrumbLink onClick={() => navigate(`/homepage`)}><ApartmentRoundedIcon />Home</BreadcrumbLink>
            {
                pathnames.map((name, index) => {
                    const isLast = index === pathnames.length -1;
                    ...
                    return isLast ? (
                        <SmallText variant="purpleAccent" weight="bold">{name}</SmallText>
                    ): <BreadcrumbLink onClick={()=>navigate(-index-1)}>{name}</BreadcrumbLink>
                })
            }
          </Breadcrumbs>
      );
